...
export const Header: React.FC<Props> = ({ children, className }) => {
  ...
  const classes = twMerge(
    ...
    return (<div className="z-50">
      <div className="header fixed flex justify-start items-center w-full top-0 left-0 h-headerHeight  bg-slate-100  z-30">
      </div>
 
      <Logo className="logo-light fixed top-0 left-0 h-headerHeight  flex items-center pl-6 text-slate-50 text-[23px] z-30" />
      <Logo className="logo-dark fixed top-0 left-0 h-headerHeight flex items-center pl-6 text-slate-700 text-[23px] z-30" />
      {children}
      {/* <MobileNav />
